...
const useWorkspaceCreation = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/workspaces/create`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` }, // ✅ FormData ke saath content-type mat lagao
        body: formData,
      });
      ...
      // ✅ workspaceId fix (backend me jo bhi return ho)
      const workspaceId = data?.workspace?._id || data?._id;

      setSuccess("Workspace created successfully! Waiting for approval.");

      if (workspaceId) {
        setTimeout(() => navigate(`/workspace/processing/${workspaceId}`), 1500);
      }

      setForm({
        workspaceName: "",
        workspaceCode: "",
        role: "general_manager", // reset properly
        logo: null,
      });
    } catch (err) {
