...
function InfoRow({ label, value, isLast }) {
  return (
    <div className={`flex items-center justify-between py-2.5 px-3.5 ${!isLast ? "border-b border-blue-50" : ""}`}>
...
export default function DepartmentDetailModal({ departmentId, onClose }) {
  ...
  useEffect(() => {
    const fetchDepartment = async () => {
      ...
      setLoading(true);
      try {
        const res = await axios.get(`/departments/full-details/${departmentId}`);
  ...
  const visibleStaff = showAll ? filteredStaff : filteredStaff.slice(0, VISIBLE_ROWS);
  ...
  if (loading) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-white/70 text-sm font-semibold">Loading department…</p>
        </div>
      </div>
    );
  }
          ...
          <div className="w-[260px] flex-shrink-0 flex flex-col gap-3.5 p-4 border-r border-blue-100 bg-blue-50/60 overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <div className="relative bg-white border border-blue-200 rounded-2xl p-3 overflow-hidden">
                ...
                <p className="text-[10px] font-bold text-blue-400 uppercase tracking-widest">Staff</p>
              ...
              <div className="relative bg-white border border-blue-200 rounded-2xl p-3 overflow-hidden">
                ...
                <p className="text-[10px] font-bold text-blue-400 uppercase tracking-widest">Status</p>
            ...
            <div className="bg-white border border-blue-200 rounded-2xl overflow-hidden">
              <InfoRow label="Department" value={department?.department || "N/A"} />
              <InfoRow
                label="Head"
                value={
                  department?.deptHeadId?.name ? (
                    ...
                    <span className="text-blue-300 italic text-[11px]">Not Assigned</span>
                  )
                }
              />
              <InfoRow label="Role" value={department?.head || "N/A"} isLast />
            </div>
          ...
          <div className="flex-1 flex flex-col p-4 overflow-hidden bg-white min-w-0">
            ...
            <div className="flex items-start justify-between gap-3 mb-4 flex-wrap">
              ...
              <div className="relative flex-shrink-0">
                <svg className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-blue-300 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <table className="w-full text-xs border-collapse" style={{ tableLayout: "fixed" }}>
                  <thead className="sticky top-0 z-10">
                    <tr className="bg-blue-50 border-b-[1.5px] border-blue-200">
                      <th className="w-9 px-3 py-2.5 text-left text-[10px] font-black uppercase tracking-widest text-blue-400">#</th>
                      <th className="px-3 py-2.5 text-left text-[10px] font-black uppercase tracking-widest text-blue-400">Name</th>
                      <th className="px-3 py-2.5 text-left text-[10px] font-black uppercase tracking-widest text-blue-400">Email</th>
                    </tr>
                  </thead>
                          ...
                          >
                            ...
                            <td className="px-3 py-2.5">
                              <div className="flex items-center gap-2 min-w-0">
                                <div className={`w-7 h-7 rounded-[9px] flex items-center justify-center text-[10px] font-black flex-shrink-0 ${color.bg} ${color.text}`}>
                                  {getInitials(u.name)}
                                </div>
                        ...
                        <td colSpan="3" className="py-14 text-center">
                          <div className="flex flex-col items-center gap-2.5">
                            ...
                            <p className="text-xs text-blue-400 font-semibold">
                              {search ? "No staff match your search" : "No staff joined yet"}
                            </p>
                            {search && (
                              <button
                                onClick={() => setSearch("")}
              ...
              {hasMore && (
                <div className="flex items-center justify-between px-4 py-2.5 bg-blue-50 border-t-[1.5px] border-blue-200 mt-auto">
                  ...
                  <button
                    onClick={() => {
                      setShowAll((v) => !v);
                      if (showAll && scrollRef.current) scrollRef.current.scrollTop = 0;
                    }}
